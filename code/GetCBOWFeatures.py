...
def train_model():
    ...
    '''
    print('train the cbow models ...')
    totxt.get_text(r'../files/all_data.xlsx')
    sentences = word2vec.Text8Corpus(r'..\files\all_2.txt')
    word2vec.Word2Vec(sentences, min_count=0, size=128, window=10, compute_loss=True, iter=200, workers=8, callbacks=[EpochLogger("l2")])
    ...
    word2vec.Word2Vec(sentences, min_count=0, size=512, window=10, compute_loss=True, iter=600, workers=8, callbacks=[EpochLogger("l4")])
