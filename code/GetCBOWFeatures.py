...
def train_model():
    sentences = word2vec.LineSentence(r'..\files\all_2.txt')
    word2vec.Word2Vec(sentences, min_count=0, size=128, window=10, compute_loss=True, iter=400, workers=8, callbacks=[EpochLogger("l2")])
    sentences = word2vec.LineSentence(r'..\files\all_3.txt')
    word2vec.Word2Vec(sentences, min_count=0, size=256, window=10, compute_loss=True, iter=400, workers=8, callbacks=[EpochLogger("l3")])
    sentences = word2vec.LineSentence(r'..\files\all_4.txt')
    word2vec.Word2Vec(sentences, min_count=0, size=512, window=10, compute_loss=True, iter=400, workers=8, callbacks=[EpochLogger("l4")])
...
class EpochLogger(CallbackAny2Vec):
    ...
    def on_epoch_end(self, model):
        ...
        print('model {} Loss after epoch {}: {}'.format(self.name, self.epoch, loss))
        self.losses.append(loss)
        np.save(r'../files/{}_losses_add.npy'.format(self.name), self.add_losses)
        self.epoch += 1
        self.previous_losses = loss
        self.add_losses.append(loss - self.previous_losses)
        if loss - self.previous_losses <= 0:
